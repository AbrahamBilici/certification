...
function App() {
  ...
  const [title, setTitle] = useState(urlsets.videogame.title);
  const [description, setDescription] = useState(urlsets.videogame.description);
  ...
  useEffect(() => {
    ...
    d3.json(dataset.url).then((data) => {
      ...
      const tooltip = d3.select('.App').append('div').attr('id', 'tooltip').style('position', 'absolute').style('opacity', 0);
  ...
  return (
    <div className="App">
      <a href='?data=videogame' >Video Game Data Set</a>  |
      <a href='?data=movie' >Movies Data Set</a> |
      <a href='?data=kickstarter'>Kickstarter Data Set</a>
      <h1 id="title">{title}</h1>
      <div id='description'>{description}</div>
      <svg id="tree-map" width={w} height={h}></svg>
